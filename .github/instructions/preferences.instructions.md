...
- 2026-04-12: Apply optional chaining/null-safe access consistently in both renderer and main-process code (`src/main/**`, `src/main/**/__tests__/**`) instead of chained `&&` guard expressions.
- 2026-04-12: Do not `await` non-thenable values in tests or runtime code. Await only Promises/thenables; call known synchronous APIs directly.
- 2026-04-12: Treat Sonar cognitive-complexity findings (`S3776`) as refactor-required for modified code paths: extract parsing/matching/tree-construction helpers rather than adding inline nested branches.
- 2026-04-12: Extract nested ternary expressions into explicit statements in both renderer and main-process modules; avoid nested ternary chains in parsing and normalization logic.
- 2026-04-12: For escaped regex/string literals in new or modified code, prefer `String.raw` for readability; for simple quote replacement patterns, use direct string-literal replacement (for example `replaceAll('"', '""')`) rather than unnecessary escape-heavy regex patterns.
- 2026-04-12: For escaped regex/string literals in new or modified code, prefer `String.raw` for readability; for simple quote replacement patterns, use direct string-literal replacement (for example `replaceAll('"', '""')`) rather than unnecessary escape-heavy regex patterns.
- 2026-04-12: Do not duplicate utility functions across proxy service files. Shared primitives (`clone`, `toText`, `normalizeHeaders`, cookie-line helpers) must live in `src/main/proxy/http-utils.js` and be imported. Before adding a helper to a new proxy service file, check `http-utils.js` first.
- 2026-04-12: Config normalization logic and default constant values must have a single source of truth. `src/main/proxy/protocol-support.js` owns `normalizeForwardRuntimeConfig`, `DEFAULT_TOOL_IDENTIFIER_HEADER`, and `DEFAULT_TOOL_IDENTIFIER_VALUE`. `src/main/index.js` must import and alias these rather than re-declaring duplicates. Never define the same string constant in two files.
- 2026-04-13: **Code Duplication Target: < 3%** — All source code in `src/` must maintain less than 3% code duplication. Baseline audit performed on 2026-04-13 showed 4.2% duplication. Consolidation roadmap documented in `instructions/CODE_DUPLICATION_AUDIT.md`. Merge-blocking criteria: any new code that introduces net duplication increase > 0.1% must be refactored to use shared utilities before approval. Before adding new utility functions, utility helpers, or constants, search `src/main/proxy/http-utils.js`, `src/main/proxy/protocol-support.js`, and existing modules for single source of truth. Code review checklist: (1) Is this function/constant already defined elsewhere? (2) Should it be shared in http-utils.js or protocol-support.js? (3) If intentional duplication, document the reason in a code comment.
